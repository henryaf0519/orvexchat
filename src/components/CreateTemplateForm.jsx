      ...
      if (file.size > MAX_SIZE_BYTES) {
        setFileError(
          `El archivo es demasiado grande (${(
            file.size /
            (1024 * 1024)
          ).toFixed(2)} MB). El máximo permitido es ${MAX_SIZE_MB} MB.`
        );
        e.target.value = null;
        setFormData((prev) => ({ ...prev, headerBase64: null }));
        return;
      }
      setFormData((prev) => ({ ...prev, headerBase64: file }));
    }
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              disabled={isEditing}
              className="mt-1 w-full px-4 py-2 border rounded-lg disabled:bg-gray-200 disabled:cursor-not-allowed"
              placeholder="ejemplo_de_promo_verano"
              required
            />
            <p className="text-xs text-gray-500 mt-1">
              Solo minúsculas, números y guiones bajos. No se puede cambiar
              después de crear.
            </p>
          </div>
        ...
        {headerType === "TEXT" && (
          <div className="mt-4 space-y-2">
            <input
              ...
              placeholder="Usa {{1}} para una variable."
            />
            {headerText.includes("{{") && (
              <input
                type="text"
                ...
                placeholder="Ejemplo para la variable {{1}}"
              />
            )}
          </div>
        )}
          ...
          <div className="mt-4 space-y-2">
            ...
            {fileError ? (
              ...
              <p className="text-xs text-gray-500">
                Recomendado: PNG o JPG de máximo{" "}
      ...
      <div className="p-4 border rounded-md bg-white shadow-sm">
        ...
        <textarea
          ...
          rows="5"
          placeholder="Escribe tu mensaje. Usa {{1}}, {{2}} para variables."
          required
        />
        {bodyVariableCount > 0 && (
          <div className="mt-4 space-y-2">
      ...
      <div className="p-4 border rounded-md bg-white shadow-sm">
        ...
        <input
          ...
          maxLength="60"
          className="w-full mt-2 px-4 py-2 border rounded-lg"
          placeholder="Texto corto y sutil"
        />
      </div>

      <div className="p-4 border rounded-md bg-white shadow-sm">
        <label className="font-semibold text-lg text-gray-800">
          Botones (Opcional, máx. 5)
        </label>
            ...
            <div key={index} className="p-3 bg-gray-50 border rounded-lg">
              ...
              {button.type === "PHONE_NUMBER" && (
                <input
                  ...
                  onChange={(e) =>
                    handleButtonChange(index, "phone_number", e.target.value)
                  }
                  className="w-full mt-2 px-3 py-2 border rounded-md text-sm"
                />
              )}
